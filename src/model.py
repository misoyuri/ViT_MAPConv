...
class UpsampleConcat(nn.Module):
    ...
    def forward(self, layer_num, dec_feature, enc_feature, dec_mask, enc_mask):
        ...
        if layer_num == 6:
            ...
            out_mask = self.upsample_for_5(dec_mask)
        else:
...
class PConvUNet(nn.Module):
    ...
    def forward(self, img, mask):
        ...
        # print("enc {}] img: {} | mask: {}".format(0, img.shape, mask.shape))
        for layer_num in range(1, self.layer_size+1):
            ...
            # print("enc {}] feature: {} | update_mask: {}".format(layer_num, feature.shape, update_mask.shape))

        assert len(enc_f) == self.layer_size

        for layer_num in reversed(range(1, self.layer_size+1)):
            # print("dec {}] feature: {} | update_mask: {}".format(layer_num, feature.shape, update_mask.shape))
